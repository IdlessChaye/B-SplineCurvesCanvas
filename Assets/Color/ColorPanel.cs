...
public class ColorPanel : MonoBehaviour, IPointerDownHandler, IDragHandler
{
	...
	void IPointerDownHandler.OnPointerDown(PointerEventData e)
	{
        ...
        Vector3 wordPos;
        //将UGUI的坐标转为世界坐标  
        if (RectTransformUtility.ScreenPointToWorldPointInRectangle(rt, e.position, e.pressEventCamera, out wordPos))
	...
	void IDragHandler.OnDrag(PointerEventData e)
	{
		...
		var pos = circleRect.anchoredPosition;
		circleRect.anchoredPosition = new Vector2(Mathf.Clamp(pos.x, 0, 255), Mathf.Clamp(pos.y, 0, 255));
